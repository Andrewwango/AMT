...
for i = 1:1:length(cs)
    ...
    % Obtain wft
    overlap_factor = 2;
    chunk_start = (c-1) * t_c / t_w * overlap_factor;
    chunk = x_ds(t_c*fs*(c-1)+1:t_c*fs*c+2);
       
    wft = abs(WindowFT(chunk, t_w*fs, t_w*fs/overlap_factor, 'Gaussian'));
...
% figure
% pattern1 = partialpattern.all_harmonies_to_pattern(all_harmonies);
return
for i = 1:1:length(cs)
    harmonies = all_harmonies{i};
    for h=1:1:length(harmonies)
        new_chord = chord(harmonies(h)).fit(harmonies(h), pattern1, pattern2);
